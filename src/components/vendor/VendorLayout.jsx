          ...
          <div className="h-full overflow-y-auto px-3 py-6">
            <nav className="space-y-1.5">
              {sidebarItems.map((item) => {
                // Exact match for dashboard, partial match for others (like /vendor/products/add matches /vendor/products)
                const isActive = item.path === '/vendor/dashboard' 
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 rounded-r-lg px-4 py-3 text-sm transition-all border-l-4 ${
                      isActive 
                        ? 'border-[#F97316] bg-orange-50 text-[#F97316] font-extrabold' 
                        : 'border-transparent text-slate-600 hover:bg-slate-50 hover:text-[#1E3A8A] font-bold'
                    }`}
                  >
                    <Icon className={`h-5 w-5 ${isActive ? 'text-[#F97316]' : 'text-slate-400'}`} />
                    {item.name}
                  </Link>
                );
              })}
